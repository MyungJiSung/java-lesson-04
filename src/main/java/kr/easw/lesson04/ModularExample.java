...
/**
 * 해당 클래스는 Vehicle 클래스를 상속해,
 * 원하는 값을 넣어 주행에 성공하는것을 목표로 하고 있습니다.
 * 해당 문제에서는 Vehicle 클래스를 상속하는 새 클래스를 선언하는것과
 * getVehicle 클래스를 수정하는것만이 허용됩니다.
 * 이 문제는 다음의 조건을 따라야 합니다 :
 * - 자신만의 새 클래스를 생성합니다. 새 클래스는 Vehicle을 상속받아야 합니다.
 * - 구현된 Vehicle을 상속한 클래스를 getVehicle 메서드가 반환하도록 수정해야 합니다.
 * - (선택) 수행률이 100% 이상이여야 합니다. 수행률은 최대 틱까지 진행이 되었는지의 척도이며,
 * - 연료가 부족할 경우 틱이 중단됩니다.
...
public class ModularExample {
    ...
    public static void main(String[] args) {
        ...
        int totalEnergy = 0;
        int tickUsed;
        int leftTick = type.tickModify() + vehicle.getEnergy().tickModify();
        for (tickUsed = 0; tickUsed < MAX_TICK; tickUsed++) {
            if (leftTick <= 0 || leftFuel <= 0) {
                break;
            }
            Energy energy = vehicle.getEnergy();
            vehicle.onTick(tickUsed, leftFuel);

            if (leftFuel < energy.fuelUsage()) {
                break;
            }
            leftFuel -= energy.fuelUsage();
            ...
            leftTick = Math.max(0, type.tickModify() + vehicle.getEnergy().tickModify());
        }
    ...
    static abstract class Vehicle {
        // Energy는 사용할 에너지를 뜻합니다. (CoalEnergy, HumanEnergy, SunlightEnergy )
        public abstract Energy getEnergy();
        // VehicleType은 탈것의 타입입니다. 타입에 따라 틱당 소모 비용이 다릅니다.
        public abstract VehicleType getType();
        // 각 틱이 실행되기 전에 실행됩니다.
        // 이 메서드를 통해 조금 더 나은 결과를 도출할 수 있습니다.
        public abstract void onTick(int currentTick, int fuel);
    }
        static class SolarCar extends Vehicle {
            @Override
    ...
    static class Bike implements VehicleType {      // 자전거 연료소모 0, 틱 -2
        @Override
        public int getCost() {
            return 0;
        }
    ...
    static class Car implements VehicleType { // 자동차 연료소모 7, 틱 2
        @Override
        public int getCost() {
            return 7;
        }
